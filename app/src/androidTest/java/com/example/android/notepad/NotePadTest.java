...
//        solo.clickOnView(solo.getView(com.example.android.notepad.R.id.menu_add));
    ...
    @Ignore
    public void addNoteFromPaste() {
        //Create text for the clipboard
        String textForPaste = "***This text is from the clipboard***" ;
        ClipboardManager cm = (ClipboardManager) solo.getCurrentActivity().getSystemService(Context.CLIPBOARD_SERVICE);
        cm.setText(textForPaste);

        //solo.sleep(3000);
        solo.clickOnMenuItem("Paste");
        solo.goBack();
        ...
        for (int i = 0; i <= 1; i++) {
            ...
            solo.clickOnView(solo.getView(R.id.menu_add));
            solo.clickLongOnView(solo.getView(R.id.note));
            solo.clickOnScreen(85, 147);
            solo.goBack();
        }
        deleteNotes();
        solo.clickLongInList(1);
